...
onMounted(async () => {
  try {
    ...
    (window as any)[`_xls_handlers_${containerId}`] = { ready: readyHandler, save: saveHandler };
...
onUnmounted(() => {
  ...
  // 清理事件监听
  const handlers = (window as any)[`_xls_handlers_${containerId}`];
  if (handlers) {
      ...
      delete (window as any)[`_xls_handlers_${containerId}`];
  }
  if (mediaQueryList) {

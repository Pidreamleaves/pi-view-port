...
export let ONLYOFFICE_BASE_URL = (import.meta.env.VITE_PI_ONLYOFFICE_BASE_URL as string) || '/packages/onlyoffice';

export const setOnlyOfficeBaseUrl = (url: string) => {
    ONLYOFFICE_BASE_URL = url.replace(/\/$/, '');
};

export const ONLYOFFICE_RESOURCE = {
    get DOCUMENTS() { return `${ONLYOFFICE_BASE_URL}/7/web-apps/apps/api/documents/api.js` },
    get X2T() { return `${ONLYOFFICE_BASE_URL}/7/wasm/x2t/x2t.js` },
    get XLSX() { return `${ONLYOFFICE_BASE_URL}/7/libs/sheetjs/xlsx.full.min.js` },
}
